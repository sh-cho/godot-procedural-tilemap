...
func generate_perimeter() -> void:
	...
	# Left-most, Right-most column
	for x in [0, size.x-1]:
		for y in range(0, size.y):
			_tile_map.set_cell(x, y, _pick_random_texture(Cell.OUTER))
	
	# First, Last row (except first, last cell)
...
func _get_random_tile(probability: float) -> int:
	var cell_type: int = Cell.GROUND if _rng.randf() < probability else Cell.OBSTACLE
	return _pick_random_texture(cell_type)
